...
`;
    ...
    if (isClient) {
        ...
        const mobileHeaderStr = '<div class="dash-mobile-header" style="display: none; justify-content: space-between; align-items: center; padding: 1rem 1.5rem; background: #080d1f; border-bottom: 1px solid rgba(255,255,255,0.05); width: 100%; box-sizing: border-box;">';
        if (html.includes(mobileHeaderStr)) {
            // Remove the logo and text in mobile header
            const logoStart = html.indexOf('<a href="dashboard_client.html">', html.indexOf(mobileHeaderStr));
            const logoEnd = html.indexOf('</a>', logoStart) + 4;
            html = html.substring(0, logoStart) + html.substring(logoEnd);
        }
    } else {
        const mobileHeaderStr = '<div class="dash-mobile-header" style="display: none; justify-content: space-between; align-items: center; padding: 1rem 1.5rem; background: #080d1f; border-bottom: 1px solid rgba(255,255,255,0.05); width: 100%; box-sizing: border-box;">';
        if (html.includes(mobileHeaderStr)) {
            // Remove the logo and text in mobile header
            const logoStart = html.indexOf('<a href="dashboard_admin.html">', html.indexOf(mobileHeaderStr));
            const logoEnd = html.indexOf('</a>', logoStart) + 4;
            if (logoStart !== -1) {
                html = html.substring(0, logoStart) + html.substring(logoEnd);
            }
