...
# Find the point to cut off
idx = content.find('<span>Storage (S3/RDS)</span><span>$15,100</span></div>')
if idx != -1:
    cut_point = content.find('\n', idx)
    content = content[:cut_point] + '\n'

rest_of_file = """                <div style="display: flex; justify-content: space-between; font-size: 0.85rem;"><span>Bandwidth & Edge</span><span>$7,600</span></div>
            </div>
